...
/*!
    ...
    This StatefulTabView doesnt destroy the removed tab, but keep it in a list.
    Also dont add new function, maintain plugin compalibility with TabView.
*/
...
FocusScope {
    ...
    */
    function insertTab(index, title, component) {
        ...
        var existingidx = existingTabIndex(title) //mine
        // https://codereview.qt-project.org/#change,65788
        var tab;
        if ( existingidx >= 0 ) {
            tab = __hiddentabs.get(existingidx).tab
            __hiddentabs.remove(existingidx, 1)
        }
        else
        {
            tab = tabcomp.createObject(loader)
    ...
    function removeTab(index) {
        ...
        tab.__inserted = false // mine: set flag to false
        __hiddentabs.append({tab: tab}) // mine: append to end of list
        if (currentIndex > 0)
            currentIndex--
        __setOpacities()
    }
    ...
    function getTab(index) {
        return (index >= 0 ? __tabs.get(index).tab : undefined)
    }

    function existingTabIndex(title) {
        for (var i = 0; i < __hiddentabs.count; ++i) {
            var child = __hiddentabs.get(i).tab
    ...
    function toggleTab(title) {
        if (existingTabIndex(title) >= 0) {
            addTab(title, undefined);
            currentIndex = count - 1;
        } else {
            removeTab(getTabIndexByTitle(title));
        }
    }
    ...
    function activateTab(title) {
        ...
        if (existingTabIndex(title) >= 0) {
            ...
            index = getTabIndexByTitle(title);
        }
        currentIndex = index;
    }
    ...
    property ListModel __hiddentabs: ListModel { }
    ...
    function __setOpacities() {
        var child
        for (var i = 0; i < __tabs.count; ++i) {
            child = __tabs.get(i).tab
            child.visible = (i == currentIndex ? true : false)
        }
        for (var j = 0; j < __hiddentabs.count; ++j) {
            child = __hiddentabs.get(j).tab
            child.visible = false
        }
        count = __tabs.count
    }
