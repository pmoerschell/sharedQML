...
Item {
    ...
    readonly property string itemType: "chassis_board"

    property int idx: -1
    property alias label: boardLabel.text
    property string fullName
    property bool used: true

    function unitImage()
    {
        console.log("Board " + fullName + " used = " + used);
        if (used) {
            if (bgMouseArea.containsMouse)
                return enabled ? "images/chassis_board_hover.png" :
                                 "images/chassis_board_disabled_hover.png";
            else
                return enabled ? "images/chassis_board.png" :
